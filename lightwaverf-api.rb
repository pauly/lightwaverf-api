...
def authorised params
  return File.exists?( File.dirname( __FILE__ ) + '/config/keys/' + params['key'].to_s )
end
  ...
  config['room'].each do | room |
    if room['name'] == params['room']
      ...
      room['device'].each do | device |
        if device['name'] == params['device']
          result = lightwaverf.send params['room'], params['device'], status, true
          return { result: result }.to_json
        end
      end
    end
